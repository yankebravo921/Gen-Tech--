// Atoms — Eyebrow, Button, Pill, Tag, ScarcityCounter, Diagram bits.
// Loaded once per page before any component script.

const Eyebrow = ({ children, color = 'orange', style }) => {
...
const Button = ({ children, variant = 'primary', size = 'md', onClick, href, style }) => {
  ...
  return <button className={cls} onClick={onClick} style={{ ...style, backgroundColor: "rgb(31, 216, 51)" }}>{children}</button>;
};

const Pill = ({ children, active, onClick, style }) =>
<button onClick={onClick} style={{
  border: 'none', cursor: 'pointer', background: active ? 'var(--gt-blue)' : 'transparent',
  color: active ? '#fff' : 'var(--gt-fg-1)', fontFamily: 'var(--gt-font-body)',
  fontWeight: 500, fontSize: 14, padding: '8px 16px', borderRadius: 999,
  transition: 'all 150ms cubic-bezier(.2,.7,.2,1)',
  ...style
}}>{children}</button>;


const Tag = ({ children, tone = 'orange', style }) => {
  const stylesMap = {
  ...
  return <span style={{
    ...
    padding: '4px 10px', borderRadius: 999, background: stylesMap.bg, color: stylesMap.fg,
    fontFamily: 'var(--gt-font-body)', letterSpacing: '0.01em', ...style
  }}>{children}</span>;
};

// A live "spots remaining" counter — Hormozi scarcity, but quiet.
const ScarcityCounter = ({ remaining = 1, total = 3, month = 'May', style }) =>
<div style={{
  display: 'inline-flex', alignItems: 'center', gap: 14,
  background: 'var(--gt-bg-card)', border: '1px solid var(--gt-border)',
  borderRadius: 999, padding: '8px 16px 8px 12px',
  boxShadow: 'var(--gt-shadow-xs)', ...style
}}>
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <span style={{ fontFamily: 'var(--gt-font-mono)', fontSize: 11, color: 'var(--gt-fg-3)', letterSpacing: '0.02em' }}>
        LIVE
      </span>
    </span>
    <span style={{ width: 1, height: 14, background: 'var(--gt-border)' }} />
    <span style={{ fontFamily: 'var(--gt-font-body)', fontSize: 13, color: 'var(--gt-fg-2)' }}>
      <span style={{ fontWeight: 600, color: 'var(--gt-fg-1)' }}>{remaining}</span> of {total} {month} spots left
    </span>
  </div>;
...
// Tiny inline icon set — lucide-flavored, hand-drawn SVG.
...
Object.assign(window, { Eyebrow, Button, Pill, Tag, ScarcityCounter, HRule, Icon });